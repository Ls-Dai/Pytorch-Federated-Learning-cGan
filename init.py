...
def load_datasets(clients, config, dataset_name):
    train_dataset = dataset_sel(dataset_name)
...
def init_federated():
    ...
    if os.path.exists(server.model_dir + server.generator_name) and os.path.exists(
        ...
        raise EOFError
    # load datasets
    load_datasets(clients=clients, config=config, dataset_name=config.dataset)
